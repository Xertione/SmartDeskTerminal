...
void HAL_FSMC_MspInit(void)
{
  ...
  PD14  ------> FSMC_D0				  PE11   ------> FSMC_D8
  PD15  ------> FSMC_D1            PE12   ------> FSMC_D9
  PD0   ------> FSMC_D2            PE13   ------> FSMC_D10
  PD1   ------> FSMC_D3            PE14   ------> FSMC_D11
  PE7   ------> FSMC_D4            PE15   ------> FSMC_D12
  PE8   ------> FSMC_D5            PD8    ------> FSMC_D13
  PE9   ------> FSMC_D6            PD9    ------> FSMC_D14
  PE10  ------> FSMC_D7            PD10   ------> FSMC_D15  

  PD11   ------> FSMC_A16  - LCD_RS
  PD4   ------> FSMC_NOE - LCD_RD
  PD5   ------> FSMC_NWE -LCD_WR
  PD7   ------> FSMC_NE1 - LCD_CS
  */
...
// 背光引脚
	__HAL_RCC_LCD_Backlight_CLK_ENABLE;		// 初始化时钟	

	LCD_Backlight_OFF;		// 背光引脚输出低，关闭背光

	GPIO_InitStruct.Pin 		= LCD_Backlight_PIN;				// 引脚
...
// 复位引脚
	__HAL_RCC_LCD_RST_CLK_ENABLE;		// 初始化时钟	
	
	GPIO_InitStruct.Pin 		= LCD_RST_PIN;					// 引脚
...
*	入口参数: 无
*	返 回 值: 无
*	函数功能: 初始化底层IO口，会在 HAL_SRAM_Init() 被调用
*	说    明: 无
******************************************************************************************/
...
void LCD_Init(void)
{
	...
	MX_FSMC_Init();	// 初始化FSMC配置
	
  // 复位ILI9488
	LCD_RST_L;	LCD_Delay(10); 	
	LCD_RST_H;	LCD_Delay(10);
