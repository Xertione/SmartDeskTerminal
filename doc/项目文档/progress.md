...
- 当前阶段：**Phase 3+4（FreeRTOS + 任务通信）实机验证通过**
- 状态：三任务并发调度正常；触摸交互在 RTOS 下可用；屏上心跳与堆余量正常刷新
- 最后更新：2026-09-23
...
- 🟡 **调试打点残留**：`main.c` 里 y=270 的 `pre-Sched` 与 y=290 的 `TOS/E` 打点是本次排查留下的，后者与心跳行（也是 y=290）位置冲突 —— 问题已定位，可清理
- 🟡 **`SystemInit()` 不设 `SCB->VTOR`**（保持复位值 0，靠 F4 的 flash 别名侥幸工作）→ 建议在 `main()` 里补 `SCB->VTOR = 0x08000000;`

---

## 下一步计划

1. 清理调试打点（`pre-Sched` / `TOS` 行；故障现场取证块**保留**，它是常驻的诊断通道）
...
- 构建：SUCCESS，0 warning，Flash **17364 B** / RAM **8788 B**（含 FreeRTOS + 8KB 堆）
- 屏上可见：`SmartDesk v0.8.0-rtos` + `Build <编译时间>` + RTOS/LCD/Touch 信息行 + SYSCLK + 触摸按钮 + Hits 计数 + Heap free + 心跳行 `HB T:… L:…`
